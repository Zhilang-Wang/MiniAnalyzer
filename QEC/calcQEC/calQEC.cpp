...
int main(int argc, char* argv[]) {
    ...
    // --- Using TChain to support wildcards (*) ---
    ...
    TH1D* h_qec_charged_in  = new TH1D("qec_charged_in",  "QEC: J/#psi inside AK8 (all charged);cos#chi", 20, -1, 1);
    TH1D* h_qec_charged_out = new TH1D("qec_charged_out", "QEC: J/#psi outside AK8 (all charged);cos#chi", 20, -1, 1);
    ...
    TH1D* h_qec_jet_in_charged  = new TH1D("qec_jet_in_charged",  "QEC: J/#psi inside AK8 (jet charged dau);cos#chi", 20, -1, 1);
    TH1D* h_qec_jet_in_neutral  = new TH1D("qec_jet_in_neutral",  "QEC: J/#psi inside AK8 (jet neutral dau);cos#chi", 20, -1, 1);
    TH1D* h_qec_jet_all         = new TH1D("qec_jet_all",         "QEC: J/#psi vs. AK8 jet (all dau);cos#chi", 20, -1, 1);

    int nTotal = 0, nInJet = 0, nOutJet = 0;
    ...
    for (Long64_t i = 0; i < nEntries; ++i) {
        ...
        if (jpsiInAnyJet) nInJet++;
        else nOutJet++;

        // Process Global Charged Hadrons
        if (ch_pt) {
            for (size_t j = 0; j < ch_pt->size(); ++j) {
                ...
                if (jpsiInAnyJet) h_qec_charged_in->Fill(c, w); 
                else              h_qec_charged_out->Fill(c, w); 
            }
        }

        // Process Jet Daughters
        if (ak8_dau_pt) {
            for (size_t idau = 0; idau < ak8_dau_pt->size(); ++idau) {
                ...
                if (jpsiInAnyJet) {
                    ...
                    if (ak8_dau_charge && ak8_dau_charge->at(idau) != 0)
    ...
    cout << "Inside AK8   : " << nInJet << endl;
    cout << "Outside AK8  : " << nOutJet << endl;
    cout << "Frac inside  : " << (double)nInJet / nTotal * 100.0 << " %" << endl;
    cout << "========================================" << endl;

    // --- Save and Close ---
    TFile* fout = new TFile(argv[2], "RECREATE");
    h_qec_charged->Write();
    h_qec_charged_in->Write();
    h_qec_charged_out->Write();
    h_qec_jet_out->Write();
    h_qec_jet_in->Write();
    ...
    cout << "Success: Results saved to " << argv[2] << endl;
    return 0;
}